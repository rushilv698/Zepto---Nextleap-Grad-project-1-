...
def _explain_evidence_rating(rating: str, conf: float | None, bd: dict) -> str:
    """Build a plain-english 'why this rating' explanation for evidence-linked insights,
    grounded in which of the 5 validation checks passed/failed."""
    checks = []
    checks.append(
        "✓ **Evidence** — 5+ supporting quotes cited" if bd.get("evidence_pass")
        else "✗ **Evidence** — fewer than 5 supporting quotes cited"
    )
    ns = bd.get("evidence_source_count", "?")
    nb = bd.get("evidence_brand_count", "?")
    checks.append(
        f"✓ **Cross-source** — evidence spans {ns} sources and {nb} brands (≥ 2 each)"
        if bd.get("cross_source_pass")
        else f"✗ **Cross-source** — only {ns} source(s), {nb} brand(s); needs ≥ 2 of each"
    )
    ua = bd.get("unique_authors", "?")
    tf = bd.get("theme_fraction", "?")
    checks.append(
        f"✓ **Statistical** — {ua} unique authors, {tf} of corpus (both above threshold)"
        if bd.get("statistical_pass")
        else f"✗ **Statistical** — only {ua} unique authors (needs ≥ 20)"
    )
    ics = bd.get("intra_cluster_sim", "?")
    checks.append(
        f"✓ **Cluster quality** — intra-cluster cosine {ics} (≥ 0.70)"
        if bd.get("cluster_quality_pass")
        else f"✗ **Cluster quality** — intra-cluster cosine {ics} (needs ≥ 0.70)"
    )
    cv = bd.get("critic_verdict") or "?"
    critic_line = {
        "pass": "✓ **LLM Critic** — verdict *pass*",
        "revise": "◐ **LLM Critic** — verdict *revise* (real signal, needs tightening)",
        "reject": "✗ **LLM Critic** — verdict *reject*",
    }.get(cv, f"**LLM Critic** — verdict *{cv}*")
    checks.append(critic_line)

    conf_str = f"{conf:.0f}" if conf is not None else "?"
    thresholds = {"high": "≥ 65", "medium": "40–64", "low": "< 40"}
    header = (
        f"**Composite confidence score: {conf_str}/100** → Rating **{rating.title()}** "
        f"(threshold: {thresholds.get(rating, '?')}). "
        "The rating is derived from the five validation checks below:"
    )
...
def _unified_insights() -> list[dict]:
    ...
    if not hyps.empty:
        for _, h in hyps.iterrows():
            rating_str = h["confidence"] if h["confidence"] in ("high", "medium", "low") else "medium"
            hyp_reason = (
                f"Rating **{rating_str.title()}** reflects the model's own honest confidence in this pattern. "
                "Working hypotheses are not gated by the 5-check validation matrix (that gate applies to "
                "theme-cluster insights). GPT-4.1 was instructed to be honest about how strongly the "
                "corpus-wide evidence supports each hypothesis — including whether counter-evidence exists."
            )
            out.append({
                "title": h["title"],
                "rating": rating_str,
    ...
    if not v2.empty:
        ...
        for _, r in primary.iterrows():
            ...
            rating_reason = _explain_evidence_rating(rating, r["confidence"], bd)
            out.append({
                "title": r["theme"],
                "rating": rating,
...
def _render_card(ins: dict, rank: int | None = None) -> None:
    ...
    with st.container(border=True):
        ...
        if ins.get("rating_reason"):
            with st.expander(f"Why this rating? ({RATING_LABEL.get(ins['rating'], '')})"):
                st.markdown(ins["rating_reason"])
...
1. **Evidence** — the generator must have cited ≥ 5 supporting review IDs.
   Insufficient citations → shelved.

2. **Cross-source** — the cited evidence must span ≥ 2 different sources (e.g., Play Store,
   Reddit, YouTube) AND ≥ 2 different brands (Zepto + at least one competitor).
   Same-source evidence gets marked *exploratory* rather than *confirmed*.

3. **Statistical** — the underlying theme cluster must have ≥ 20 unique authors AND cover
   at least 0.5% of the filtered corpus. Prevents fringe hypotheses from a handful of loud voices.

4. **Cluster quality** — the theme's supporting reviews must have a mean intra-cluster cosine
   similarity ≥ 0.70. Ensures the reviews are actually about the same thing — a low score means
   the cluster is semantically noisy and the insight would be hollow.

5. **LLM Critic Agent** — GPT-4.1 (a different model from the generator) reads the insight
   plus the supporting evidence plus 10 retrieved *counter-evidence* snippets, and returns
   a verdict: **pass** / **revise** / **reject**. Rejects are shelved from the Dashboard.

**Two design choices worth calling out**

- **The Critic is a DIFFERENT model from the generator.** Insights are generated by DeepSeek;
  the Critic is GPT-4.1. Same model marking its own homework is worthless — a fresh model
  gives an independent read.

- **The Critic sees retrieved counter-evidence, not just supporting quotes.** For each insight,
  the pipeline searches the corpus for snippets that could *contradict* the hypothesis and
  hands them to the Critic. Verdict is delivered against that adversarial context.
